...
export default function ProductPage() {
    ...
    useEffect(() => {
        ...
        fetch(`/api/products/${encodeURIComponent(slug)}`, { cache: 'no-store' })
            ...
            .then(d => {
                if (d.success) {
                    setProduct(d.data);
                    // Do not pre-select a color variant on initial page load
                    setSelectedVariant(null);
                }
                setLoading(false);
            })
    ...
    const currentStockStatus = currentVariant ? (currentVariant.stock > 0 ? "IN_STOCK" : "OUT_OF_STOCK") : product.stockStatus;
                        ...
                        <div className="space-y-5">
                            ...
                            {/* <div className="flex items-center gap-2">
                                {currentStockStatus === "IN_STOCK" ? (
                                    <><Check className="h-4 w-4 text-green-500" /><span className="text-green-600 font-medium text-sm">In Stock ({currentStock} available)</span></>
                                ) : currentStockStatus === "MADE_TO_ORDER" ? (
                            ...
                            {/* Color selection is now displayed below image/thumbnails in the left column */}
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 pt-4 border-t">
                             ...
                             {/* Available Colors Card - Positioned right under the benefits grid */}
                                     ...
                                     <div className="flex flex-wrap gap-3">
                                         {activeVariants.map((v: ProductVariant, index: number) => {
                                             ...
                                             const COLOR_LOOKUP: Record<string, string> = {
                                                 "chrome": "linear-gradient(135deg, #CFD8DC 0%, #ECEFF1 50%, #90A4AE 100%)",
                                                 "silver": "linear-gradient(135deg, #CFD8DC 0%, #ECEFF1 50%, #90A4AE 100%)",
                                                 "matt black": "linear-gradient(135deg, #1A1A1A 0%, #3D3D3D 50%, #1A1A1A 100%)",
                                                 "black": "linear-gradient(135deg, #1A1A1A 0%, #3D3D3D 50%, #1A1A1A 100%)",
                                                 "rose gold": "linear-gradient(135deg, #ECC4B2 0%, #ECC4B2 30%, #C88E72 100%)",
                                                 "gold": "linear-gradient(135deg, #F9D976 0%, #E9B646 50%, #C1932E 100%)",
                                                 "antique gold": "linear-gradient(135deg, #D4AF37 0%, #AA7C11 100%)",
                                                 "brushed steel": "linear-gradient(135deg, #B0BEC5 0%, #ECEFF1 50%, #78909C 100%)",
                                                 "grey": "linear-gradient(135deg, #78909C 0%, #CFD8DC 100%)",
                                                 "gray": "linear-gradient(135deg, #78909C 0%, #CFD8DC 100%)",
                                                 "bronze": "linear-gradient(135deg, #CD7F32 0%, #A0522D 100%)",
                                                 "white": "linear-gradient(135deg, #FFFFFF 0%, #F5F5F5 100%)",
                                                 "red": "linear-gradient(135deg, #FF1744 0%, #D50000 100%)",
                                                 "blue": "linear-gradient(135deg, #2979FF 0%, #2962FF 100%)",
                                                 "green": "linear-gradient(135deg, #00E676 0%, #00C853 100%)",
                                                 "yellow": "linear-gradient(135deg, #FFEA00 0%, #FFD600 100%)",
                                                 "orange": "linear-gradient(135deg, #FF9100 0%, #FF6D00 100%)",
                                                 "pink": "linear-gradient(135deg, #FF4081 0%, #F50057 100%)",
                                                 "purple": "linear-gradient(135deg, #D500F9 0%, #AA00FF 100%)",
                                             };
                                             
                                             let swatchBg = COLOR_LOOKUP[colorName];
                                             if (!swatchBg) {
                                                 // Generate a high-fidelity linear gradient dynamically based on the text hash of the custom color name!
                                                 let hash = 0;
